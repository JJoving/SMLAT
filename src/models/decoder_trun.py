...
class Decoder(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_dim, sos_id, eos_id, hidden_size,
                 num_layers, offset, bidirectional_encoder=True):
        ...
        if self.atype == "dot":
            ...
            self.attention = ContentBasedAttention(512)
        ...
        for t in range(output_length):
            ...
            if t + 1 < aligns_pad.size(1):
                for m in range(mask.size(0)):
                    left_bound = min(aligns_in_pad[m][t] + self.offset, rnn_output.size(1))
                    right_bound = min(aligns_in_pad[m][t + 1] + self.offset, rnn_output.size(1))
                    if self.TA:
    ...
    def recognize_beam(self, encoder_outputs, char_list, left, right, args):
        ...
        left_pad = torch.zeros(maxlen).int()
        right_pad = torch.zeros(maxlen).int()
        left_pad [:left.size(0)] = left
        right_pad [:right.size(0)] = right
        ...
        for i in range(maxlen):
            ...
            for hyp in hyps:
                ...
                for m in range(mask.size(0)):
                    left_bound = max(left_pad[i] + self.offset, rnn_output.size(1))
                    right_bound = min(right_pad[i] + self.offset, rnn_output.size(1))
                    if self.TA:
                        mask[m][0:left_bound] = 0
                    else:
                        mask[m][left_bound:right_bound] = 0
                att_c, att_w = self.attention(rnn_output.unsqueeze(dim=1),
