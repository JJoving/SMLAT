...
LFR_n=1  # Low Frame Rate: number of frames to ski
lsm_weight=0
...
dict=data/lang_1char/train_chars_ctc.txt
...
if [ ${stage} -le 4 ]; then
    ...
    decode_dir=${expdir}/decode_test_beam${beam_size}_nbest${nbest}_ml${decode_max_len}_cweight${ctc_weight}_epoch${epochs}
    mkdir -p ${decode_dir}
