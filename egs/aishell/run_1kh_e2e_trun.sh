...
dumpdir=dump_1kh_ctc2   # directory to dump full features
...
if [ ${stage} -le 4 ]; then
    ...
    decode_dir=${expdir}/decode_test_beam${beam_size}_nbest${nbest}_ml${decode_max_len}_cweight${ctc_weight}_test_set_dec
    mkdir -p ${decode_dir}
    ${cuda_cmd} --gpu ${ngpu} ${decode_dir}/decode.log \
        recognize_e2e.py \
        --recog_json ${feat_dev_dir}/data.json \
        --dict $dict \
