...
data_set=2500h
data_path=/search/odin/chenjunjie/github/Listen-Attend-Spell/egs/aishell/data/$data_set
no_cv=false
# --
...
dict=data_1kh/lang_bytes/train_bytes.dict
...
if [ $stage -le 2 ]; then
    ...
    for data in train cv; do
        ...
        data2json_bytes.sh --feat ${feat_dir}/feats.scp \
             $data_path/$data ${dict} > ${feat_dir}/data_bytes.json
    done
    #for data in 8000 ios nos; do
    #  data2json_bytes.sh --feat $feat_test_dir/${data}/feats.scp \
    #    data_1kh/test/${data} ${dict} > ${feat_test_dir}/${data}/data.json
    #done
fi

if [ -z ${tag} ]; then
    expdir=exp/$data_set/train_in${einput}_hidden${ehidden}_e${elayer}_${etype}_drop${edropout}_${atype}_emb${dembed}_hidden${dhidden}_d${dlayer}_epoch${epochs}_norm${max_norm}_bs${batch_size}_mli${maxlen_in}_mlo${maxlen_out}_${optimizer}_lr${lr}_mmt${momentum}_l2${l2}_bidirectionaltrain${ebidirectional}
    if ${do_delta}; then
        expdir=${expdir}_delta
    fi
else
    expdir=exp_1kh_bytes/train_${tag}
fi
expdir=${expdir}_bytes
...
if [ ${stage} -le 3 ]; then
    ...
    ${cuda_cmd} --gpu ${ngpu} ${expdir}/train.log \
        train.py \
        --train_json ${feat_train_dir}/data_bytes.json \
        --valid_json ${feat_cv_dir}/data_bytes.json \
        --dict ${dict} \
        --einput $einput \
        --ehidden $ehidden \
        --elayer $elayer \
...
if [ ${stage} -le 4 ]; then
  ...
  for data in 8000 ios nos; do
    ...
    ${cuda_cmd} --gpu ${ngpu} ${decode_dir}/decode.log \
        recognize.py \
        --recog_json ${feat_test_dir}/${data}/data_bytes.json \
        --dict $dict \
    ...
    # Compute CER
    local/score_bytes.sh ${decode_dir} ${dict}
  done
fi
