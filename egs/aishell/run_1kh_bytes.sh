...
dumpdir=dump_1kh_bytes   # directory to dump full features
...
if [ -z ${tag} ]; then
    expdir=exp_1kh_bytes/train_in${einput}_hidden${ehidden}_e${elayer}_${etype}_drop${edropout}_${atype}_emb${dembed}_hidden${dhidden}_d${dlayer}_epoch${epochs}_norm${max_norm}_bs${batch_size}_mli${maxlen_in}_mlo${maxlen_out}_${optimizer}_lr${lr}_mmt${momentum}_l2${l2}
    if ${do_delta}; then
